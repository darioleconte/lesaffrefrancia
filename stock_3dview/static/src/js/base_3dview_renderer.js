odoo.define('3dview.3DViewRenderer', function (require) {
    ...
    var ThreeDViewRenderer = AbstractRenderer.extend({
        ...
        activateAreaChoice: function() {
            ...
            if (area_ids.length>1) {
                area_ids.forEach(function(id) {
                    ...
                    $("#area_id").append($('<option>', { value: id, text: scope.state.data.areas[id].name, id: 'wh'+id }));
                });
                // we don't want the event to be reflected on the canvas
                $("#areas").show().bind('blur change click dblclick error focus focusin focusout hover keydown keypress keyup load mousedown mouseenter mouseleave mousemove mouseout mouseover mouseup resize scroll select submit', function(event) {
        ...
        addObject: function (item3d, type) {
        	...
        	var size = scope._realTo3dvSizes (item3d.sizex, item3d.sizey, item3d.sizez, scope.state.data.areas[item3d[scope.fields.area]].scale);
            ...
            switch (type) {
                case 'mesh':
                    ...
                    var material = new THREE.MeshPhongMaterial( { color: new THREE.Color(item3d.color), flatShading: true, opacity: parseInt(item3d.opacity, 10)/1000, transparent: true } );
                ...
                case 'wireframe':
                    ...
                    if (item3d.geometry) {
                        ...
                        customObj.traverse( function(child) {
                            if ( child.isMesh ) {
                                var wf = new THREE.LineSegments( new THREE.WireframeGeometry( child.geometry ), material );
        ...
        manageGLTFObjectsVisibility: function() {
            for (let key in scope.areaObjects.gltfObjects) {
                scope.areaObjects.gltfObjects[key].model.visible = scope.areaObjects.gltfObjects[key].area == scope.currentArea.id;
                scope.areaObjects.gltfObjects[key].light.intensity = scope.areaObjects.gltfObjects[key].area == scope.currentArea.id ? 7 : 0;
            }
        ...
        showGLTFModel: function(item3d) {
            ...
            scope.GLTFLoader.load(this.calls.gltf3dModel + item3d.gltf, function ( gltf ) {            
                    ...
                    mod.position.fromArray( scope._realTo3dvCoords (
                        parseInt(item3d.posx, 10),
                        parseInt(item3d.posy, 10),
                        parseInt(item3d.posz, 10),
                        parseInt(item3d.sizex, 10),
        ...
        findItem3d: function ( event ) {
            ...
            var intersects = scope.raycaster.intersectObjects( scope.areaObjects.meshes.filter( obj => obj.userData.area == scope.currentArea.id ));
        ...
        onContainerDoubleClick: function( event ) {
            ...
            if ( item3d ) {
                ...
                {
                    $("#item3d_info").css('left', event.clientX - $('#threedview_container').offset().left - $("#item3d_info").width() - 30 );
                }
                if ( parseInt($("#item3d_info").css("bottom"), 10 ) < 0 )
                {
                    $("#item3d_info").css('top', event.clientY - $('#threedview_container').offset().top - $("#item3d_info").height() - 30 );
                }
                ...
                ajax.jsonRpc(scope.calls.item3dInfo, 'call', {'domain': domain }).then( function (data) {
                    ...
                    */
                    ...
                    $("#frame_item3d_icon").show().click( function( event ) {
                        event.preventDefault();
        ...
        _initRefSys: function(area) {
            ...
            for (let id in scope.state.data.areas) {
                ...
                area.refSys.groundZ_3dv = Math.round( area.refSys.groundY_real / area.refSys.groundX_real * area.refSys.groundX_3dv );
                area.refSys.heightY_3dv = Math.round( area.refSys.heightZ_real / area.refSys.groundX_real * area.refSys.groundX_3dv );
